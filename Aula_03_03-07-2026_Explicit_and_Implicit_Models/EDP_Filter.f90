...
END MODULE Class_WritetoGrads
 ...
 MODULE ModAdvection
  USE Class_Fields, Only: DeltaT,DeltaX,Idim,r8,xa,tf0,tb0,yyf,xxb,xb0,xf0,Area,C,Deslc,alfa,beta
   IMPLICIT NONE
   PRIVATE
...
!   
 ...
 FUNCTION CentredTimeSpace(Q,t,dt,dx)  result(Qnp1)
   !"""
   ...
   DO i=N0,N1
      ...
      Qnp1(i) = Q(xc) - dt*( C*Q(xf) - C*Q(xb) )/2*dx
   END DO
...
PROGRAM  Main
  ...
  USE Class_WritetoGrads, Only : SchemeWriteCtl,SchemeWriteData,InitClass_WritetoGrads
   IMPLICIT NONE
   REAL(KIND=r8)               :: tend = 200.!End time of simulation
   CHARACTER(LEN=10) :: scheme      = 'RK4CS_RAW'  ! Advection scheme. Possible values: UPSTREAM, CTCS,RK4CS,RK4CS_RA,RK4CS_RAW
      ...
      ! Time stepping
       ...
       DO WHILE  (t < tend )      
          ...
          if( TRIM(scheme) == 'RK4CS')THEN
             ...
             up = CentredTimeSpace(u,t,DT,DeltaX)
          else
             scheme = 'default'
