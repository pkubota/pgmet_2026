   ...
   SUBROUTINE  Solve_Forward_Beta_plane_Grid_A(TermEqMomU, TermEqMomV, TermEqConH, u, v, h)
      ...
      DO j=1,Jdim
	    ...
	    betav = -Beta*coordY(xc,yc)*v(xc,yc)
            ...
            ! dt            H0-Bath    dx
            !
            gdbathx= g *((h(xc,yc)/(H0-Bath(xc,yc))))*((Bath(xf,yc) - Bath(xb,yc))/(2*DeltaX))
            !                                                                          -                      -    
            ...
            ! dt            H0-Bath    dy
            !
            gdbathy= g *((h(xc,yc)/(H0-Bath(xc,yc))))*((Bath(xf,yc) - Bath(xb,yc))/(2*DeltaY))
...
PROGRAM  Main
  USE ModSWE, Only :    DefineCI,Solve,u,v,h,Idim,Jdim,DeltaT,&
                        f,nt,Solve_Forward_Beta_plane,coordY,bath,Solve_Forward_f_plane,&
			Solve_Forward_Beta_plane_Sphera,Solve_Forward_Beta_plane_Grid_A			
   IMPLICIT NONE
