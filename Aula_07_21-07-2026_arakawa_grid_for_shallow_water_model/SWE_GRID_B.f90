MODULE ModSWE
  ...
  PUBLIC :: Solve,Solve_Forward_Beta_plane,Solve_Forward_f_plane,&
            Solve_Forward_Beta_plane_Sphera,Solve_Forward_Beta_plane_Grid_B
...
CONTAINS
   ...
   SUBROUTINE  Solve_Forward_Beta_plane_Grid_B(TermEqMomU, TermEqMomV, TermEqConH, u, v, h)
      ...
      REAL(KIND=8)    :: betau,betav
      DO j=1,Jdim
         CALL index_block(j,Jdim,yb,yc,yf)
         DO i=1,Idim
            CALL index(i,Idim,xb,xc,xf)
            !
            !                     
	    ...
	    betav = -Beta*coordY(xc,yc)*v(xc,yc)
            ...
            !
            !
            gdhx  = g*((((h(xc,yc)+h(xc,yb))/2.0) - ((h (xb,yc)+h (xb,yb))/2.0))/(DeltaX))
            !
            !
            !       
            !       
            ...
            !
            !
            gdhy = g * (( ((h(xc,yc)+h(xb,yc))/2.0) - ((h(xc,yb) + h(xb,yb) )/2.0))/(DeltaY))
            !
            !
            ! d(v)
            ! -----  + b*v =0
            ...
            ! dt                dx     
            !
            hdudx = H0 * (( ((u(xf,yf)+u(xf,yc))/2.0)  - ((u(xc,yf)+u(xc,yc))/2.0))/(DeltaX))
            !
            ! d(Eta)           d(v)
            ! -----  + Eta * -----------
            ! dt               dy
            !            
            hdvdy = H0 * (( ((v(xf,yf)+v(xc,yf))/2.0)  - ((v(xf,yc)+v(xc,yc))/2.0))/(DeltaY))
            !
            ! d(Eta)        d(Eta)
            ! -----  + u * ---------  =0
            ! dt             dx 
            !
	    ubar=(u(xf,yf)+u(xc,yf) + u(xf,yc)+u(xc,yc) )/4.0
	    !
            dhudx = ubar*(h(xf,yc)  - h(xb,yc))/(2*DeltaX)
            !
            ! d(Eta)          d(Eta)
            ! -----   + v * ----------- =0
            ! dt              dy    
            !
            vbar= (v(xf,yf)+v(xc,yf)+v(xf,yc)+v(xc,yc) )/4.0
            !
            dhvdy = vbar*(h(xc,yf)  - h(xc,yb))/(2*DeltaY)
            !
            !                  -         -
            ! d(u)            |d(d(h))    |
            ! -----  - Neta * |--------   | = 0
   ...
   END SUBROUTINE Solve_Forward_Beta_plane_Grid_B
...
PROGRAM  Main
  USE ModSWE, Only :    DefineCI,Solve,u,v,h,Idim,Jdim,DeltaT,&
			...
			Solve_Forward_Beta_plane_Grid_B
  ...
  OPEN(1,FILE='saida_GridB.bin',ACCESS='DIRECT',FORM='UNFORMATTED', RECL=lrec,&
...
CONTAINS
  ...
  SUBROUTINE Run(it)
      ...
      REAL(KIND=8) :: dt
        ...
        call Solve_Forward_Beta_plane_Grid_B(ku1, kv1, kh1, uo        , vo        , ho        )
        call Solve_Forward_Beta_plane_Grid_B(ku2, kv2, kh2, uo+ku1*dt2, vo+kv1*dt2, ho+kh1*dt2)
        call Solve_Forward_Beta_plane_Grid_B(ku3, kv3, kh3, uo+ku2*dt2, vo+kv2*dt2, ho+kh2*dt2)
        call Solve_Forward_Beta_plane_Grid_B(ku4, kv4, kh4, uo+ku3*dt , vo+kv3*dt , ho+kh3*dt )
