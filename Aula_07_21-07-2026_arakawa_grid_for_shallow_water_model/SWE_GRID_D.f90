MODULE ModSWE
  ...
  PUBLIC :: Solve,Solve_Forward_Beta_plane,Solve_Forward_f_plane,&
	    ...
	    Solve_Forward_Beta_plane_Grid_D
...
CONTAINS
   ...
   SUBROUTINE  Solve_Forward_Beta_plane_Grid_D(TermEqMomU, TermEqMomV, TermEqConH, u, v, h)
      ...
      DO j=1,Jdim
         ...
         DO i=1,Idim
            ...
            vbar  = 0.25*(v(xf,yc) + v(xc,yc) +   v(xf,yb)+   v(xc,yb))
	    ...
	    betav = -Beta*coordY(xc,yc)*vbar
            ...
            !
            !
            gdhx  = g* ( ( ( (h(xf,yc)+h(xf,yb))/2.0) - ((h(xb,yc)+h(xb,yb))/2.0))/(2.0*DeltaX))
            !
            !
            !       
            !       
            ...
            !
            !
            ubar=0.25*(u(xc,yf) + u(xb,yf) + u(xc,yc) + u(xb,yc) )
            ...
            !
            !
            gdhy = g * (( ( ( h(xc,yf) + h(xb,yf)  )/2.0) - ( (h(xc,yb)+h(xb,yb))/2.0))/(2.0*DeltaY ))
            !
            !
            ! d(v)
            ! -----  + b*v =0
            ...
            ! dt                dx     
            !
            hdudx = H0 * (( ( (u(xf,yf)+u(xf,yc))/2.0)  - ( (u(xb,yf)+u(xb,yc))/2.0))/(2.0*DeltaX))
            !
            ! d(Eta)           d(v)
            ! -----  + Eta * -----------
            ! dt               dy
            !            
            hdvdy = H0 * (( ( (v(xf,yf)+v(xc,yf))/2.0)  - ( (v(xf,yb)+v(xc,yb))/2.0))/(2.0*DeltaY))
            !
            ! d(Eta)        d(Eta)
            ! -----  + u * ---------  =0
            ! dt             dx 
	    ...
	    ubar= (u(xc,yf)+u(xc,yc))/2.0
         ...
         END DO
      END DO
   END SUBROUTINE Solve_Forward_Beta_plane_Grid_D
...
PROGRAM  Main
  ...
  OPEN(1,FILE='saidaD.bin',ACCESS='DIRECT',FORM='UNFORMATTED', RECL=lrec,&
...
CONTAINS
  ...
  SUBROUTINE Run(it)
      ...
      REAL(KIND=8) :: dt
        ...
        call Solve_Forward_Beta_plane_Grid_D(ku1, kv1, kh1, uo        , vo        , ho        )
        call Solve_Forward_Beta_plane_Grid_D(ku2, kv2, kh2, uo+ku1*dt2, vo+kv1*dt2, ho+kh1*dt2)
        call Solve_Forward_Beta_plane_Grid_D(ku3, kv3, kh3, uo+ku2*dt2, vo+kv2*dt2, ho+kh2*dt2)
        call Solve_Forward_Beta_plane_Grid_D(ku4, kv4, kh4, uo+ku3*dt , vo+kv3*dt , ho+kh3*dt )
